...
class GroupReferenceServiceImplTest {
    ...
    void getGroupReferenceDelegates() {
        ...
        var group = GroupEntity.builder().id(id).name("G").build();
    ...
    void getSubgroupReferenceDelegates() {
        var id = UUID.randomUUID();
        var subgroup = SubgroupEntity.builder().id(id).index(1).build();
    ...
    void resolveAudienceRejectsSubgroupFromAnotherGroup() {
        ...
        var group = GroupEntity.builder().id(groupId).name("G").build();
        var foreignGroup =
                GroupEntity.builder().id(UUID.randomUUID()).name("Other").build();
        var subgroup = SubgroupEntity.builder()
                .id(subgroupId)
                .index(1)
                .group(foreignGroup)
                .build();
