...
CREATE UNIQUE INDEX uk_permission_scopes_active
    ON teacher_subject_permission_scopes (permission_id, group_id, allowed_subgroup_id, allowed_lesson_type) NULLS NOT DISTINCT
...
ALTER TABLE check_in_sessions
    ADD COLUMN scope_id UUID REFERENCES teacher_subject_permission_scopes (id) ON DELETE CASCADE;
...
SELECT p.id,
       (SELECT p2.id
        FROM teacher_subject_permissions p2
...
SET scope_id = psm.scope_id FROM perm_scope_map psm
...
DELETE
FROM teacher_subject_permissions p USING perm_canonical pc
