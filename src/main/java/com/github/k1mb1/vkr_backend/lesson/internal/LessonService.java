...
class LessonService implements LessonApi {
        ...
        if (newScopes != null) {
            ...
            if (!effectiveAllGroups && newScopes.isEmpty()) {
                throw new IllegalArgumentException(
                    "scopes must be non-empty when allGroups=false"
                );
            }
    ...
    public List<LessonResponse> getLessons(LessonFilter filter) {
        var permission = permissionRepository
            .findByIdWithDetails(filter.permissionId())
            .orElseThrow(() ->
    ...
    public List<LessonResponse> createLessonsByType(
        ...
        for (int i = 0; i < request.lectureCount(); i++) {
            lessons.add(
                lesson(subject, request.allGroups(), resolvedScopes, LessonType.LECTURE, today)
            );
        }
        for (int i = 0; i < request.practiceCount(); i++) {
            lessons.add(
                lesson(subject, request.allGroups(), resolvedScopes, LessonType.PRACTICE, today)
            );
        }
    ...
    private List<ResolvedScope> resolveScopes(
        Boolean allGroups,
    ...
    private List<LessonScope> buildLessonScopes(
        Lesson lesson,
        List<LessonScopeRequest> requests
    ) {
        ...
        while (lessons.size() < entry.totalCount()) {
            ...
            for (var dow : sortedDays) {
                ...
                if (!lessonDate.isBefore(entry.startDate())) {
                    lessons.add(
                        lesson(subject, allGroups, scopes, entry.type(), lessonDate)
                    );
                }
            }
    ...
    private record ResolvedScope(Group group, Subgroup allowedSubgroup) {}
}
