...
record LessonSpecifications(LessonFilter filter) {
    public Specification<Lesson> toSpecification() {
        return (root, query, cb) -> {
            ...
            predicates.add(
                cb.equal(root.get("subject").get("id"), filter.subjectId())
            );

            Subquery<Long> sq = query.subquery(Long.class);
            Root<TeacherSubjectPermission> perm = sq.from(
            ...
            subPredicates.add(
                cb.equal(perm.get("teacher").get("id"), filter.teacherId())
            );
            subPredicates.add(
                cb.equal(
            ...
            Predicate subgroupPred = cb.or(
                ...
                cb.equal(perm.get("allowedSubgroup"), root.get("subgroup"))
            );
