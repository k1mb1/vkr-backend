...
class TeacherSubjectPermissionService implements TeacherSubjectPermissionsApi {
    ...
    private List<PermissionScope> buildScopes(
            TeacherSubjectPermission permission, List<PermissionScopeRequest> requests) {
        Subject subject = permission.getSubject();
        var subjectGroupIds =
                subject.getGroups().stream().map(Group::getId).collect(java.util.stream.Collectors.toSet());
