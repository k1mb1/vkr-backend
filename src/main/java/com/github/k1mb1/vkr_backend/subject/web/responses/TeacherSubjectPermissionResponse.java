...
public record TeacherSubjectPermissionResponse(
    ...
    @Schema(
        description = "Список scope'ов (группа с полным списком подгрупп + опц. разрешённая подгруппа + опц. тип занятия). " +
            "При allPermissions=true scope'ы синтезируются из групп предмета (по одному на группу, без ограничений по подгруппе и типу)."
    )
