...
public class SubjectFinalAssessmentPolicyService {
     ...
     */
    private void applyRequestToPolicy(FinalAssessmentPolicyEntity policy, FinalAssessmentPolicyRequest request) {
    ...
    private void syncBands(
            FinalAssessmentPolicyEntity policy, java.util.List<FinalAssessmentPolicyRequest.Band> requestBands) {
        var existingById = policy.getBands().stream().collect(Collectors.toMap(AssessmentBandEntity::getId, b -> b));
        ...
        for (var req : requestBands) {
            if (req.id() == null || !existingById.containsKey(req.id())) {
                var band = AssessmentBandEntity.builder()
                        .policy(policy)
                        .label(req.label())
    ...
    private void resetAttendance(FinalAssessmentPolicyEntity policy) {
     ...
     * балл через AttendancePolicyEntity, доп. полей нет. SEPARATE — отдельный гейт: обязателен
     * attendanceRequirementMode, соответствующий порог (percent/count) и хотя бы один включённый
     * статус посещения.
     */
    private void applyAttendanceMode(FinalAssessmentPolicyEntity policy, FinalAssessmentPolicyRequest request) {
