...
@Schema(description = """
        ...
        Если enabled = true — onTimeSeconds и lateSeconds обязательны (проверяется валидатором ниже,
        не только на сервисе), и любая check-in сессия предмета использует именно эти окна,
        игнорируя значения из запроса на запуск. Если enabled = false — окна задаются
        индивидуально при запуске сессии.
        """)
