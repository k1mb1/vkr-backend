...
public class StudentAttendanceService {
    ...
    public Page<StudentAttendanceResponse> findAll(
        StudentAttendanceFilter filter,
        Pageable pageable
    ) {
    ...
    public List<StudentAttendanceResponse> findAllBySubjectId(UUID subjectId) {
        return studentAttendanceRepository
            .findAllByLesson_Subject_Id(subjectId)
            .stream()
            .map(studentAttendanceMapper::toResponse)
            .toList();
    }

    public List<StudentAttendanceResponse> findAllByStudentIdAndSubjectId(
        UUID studentId,
        UUID subjectId
    ) {
        return studentAttendanceRepository
            .findAllByStudent_IdAndLesson_Subject_Id(studentId, subjectId)
            .stream()
            .map(studentAttendanceMapper::toResponse)
            .toList();
    }
    ...
    @Transactional
    public List<StudentAttendanceResponse> bulkCreate(
        UUID lessonId,
        ...
        return request
            ...
            .map(item -> {
                ...
                var entity = StudentAttendanceEntity.builder()
                    ...
                    .note(item.note())
                    .build();
                return studentAttendanceMapper.toResponse(
    ...
    @Transactional
    public StudentAttendanceResponse update(
        UUID id,
        UpdateStudentAttendanceRequest request
    ) {
        var entity = studentAttendanceRepository
            .findById(id)
            .orElseThrow(() ->
                new EntityNotFoundException("Attendance not found: " + id)
            );
