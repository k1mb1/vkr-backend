...
public interface StudentGroupMapper {
    ...
    @Mapping(target = "studentIds", expression = "java(entity.getStudents().stream().map(s -> s.getId()).toList())")
    ...
    @Mapping(target = "updatedAt", ignore = true)
    @Mapping(target = "students", ignore = true)
    void update(@MappingTarget StudentGroupEntity entity, UpdateStudentGroupRequest request);
}