...
class GroupReferenceServiceImpl
    ...
    @Override
    public AudienceRef resolveAudience(UUID groupId, UUID allowedSubgroupId) {
        if (groupId == null) {
            return new AudienceRef(null, null);
        }
        Group group = groupRepository.getReferenceById(groupId);
        Subgroup allowedSubgroup = allowedSubgroupId != null
                                   ? subgroupRepository.getReferenceById(allowedSubgroupId)
                                   : null;
