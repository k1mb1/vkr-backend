...
public interface TeacherSubjectPermissionRepository extends JpaRepository<TeacherSubjectPermissionEntity, UUID> {
    ...
    /** id предметов, на которые у преподавателя есть право — для фильтра видимости subject. */
    @Query("SELECT p.subject.id FROM TeacherSubjectPermissionEntity p WHERE p.teacher.id = :teacherId")
    List<UUID> findSubjectIdsByTeacherId(UUID teacherId);
