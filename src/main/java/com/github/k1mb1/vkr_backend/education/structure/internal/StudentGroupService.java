...
import com.github.k1mb1.vkr_backend.education.structure.api.requests.StudentGroupMemberRequest;
...
import com.github.k1mb1.vkr_backend.education.structure.api.responses.StudentGroupMemberResponse;
import com.github.k1mb1.vkr_backend.education.structure.api.responses.SubgroupResponse;
...
public class StudentGroupService {
    ...
    public Page<GroupPageResponse> findAll(StudentGroupFilter filter, Pageable pageable) {
        return groupRepository.findAll(filter.toSpecification(), pageable)
            .map(group -> new GroupPageResponse(group.getId(), group.getName(), group.getSubgroups().size()));
    }

    @Transactional
    public GroupResponse create(CreateGroupRequest request) {
        var mainGroup = StudentGroupEntity.builder().name(request.groupName()).build();
        ...
        if (distinctIndices.isEmpty()) {
            for (var member : request.students()) {
                var student = StudentEntity.builder().username(member.username()).group(mainGroup).build();
        ...
        for (var member : request.students()) {
            var student = StudentEntity.builder().username(member.username()).build();
            if (member.subgroupIndex() == null) {
                student.setGroup(mainGroup);
                mainGroup.getStudents().add(student);
            } else {
                var subgroup = indexToSubgroup.get(member.subgroupIndex());
                if (subgroup == null) throw new IllegalArgumentException("Invalid subgroupIndex: " + member.subgroupIndex());
    ...
    public GroupResponse findGroupWithSubgroups(UUID id) {
        var group = groupRepository.findWithSubgroupsAndStudentsById(id)
            .orElseThrow(() -> new EntityNotFoundException(NOT_FOUND_MESSAGE.formatted("Main group", id)));
    ...
    public void delete(UUID groupId) {
        groupRepository.delete(getMainGroupById(groupId));
    }

    private StudentGroupEntity getMainGroupById(UUID id) {
        return groupRepository.findByIdAndParentGroupIsNull(id)
            .orElseThrow(() -> new EntityNotFoundException(NOT_FOUND_MESSAGE.formatted("Main group", id)));
    }
    ...
    private GroupResponse mapToGroupResponse(StudentGroupEntity entity) {
        ...
        return GroupResponse.builder().id(entity.getId()).name(entity.getName())
            .subgroups(subgroups).students(students).build();
    }
}
